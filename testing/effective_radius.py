...
def test_single_sphere_effective_radius():
    ...
    r_meshs = [((3*v_mesh/(4*np.pi))**(1/3)) for v_mesh in v_meshs]
    v_mcs, t_mcs, dev_mcs = get_mc_volume_samples(rs, centers, nsampless)
    sems = np.array([dev_mc/v_mc for dev_mc, v_mc in zip(dev_mcs, v_mcs)])
    r_mcs = [((3*v_mc/(4*np.pi))**(1/3)) for v_mc in v_mcs]
    # For z = cx^a, δz = a*(z/x)*δx
    rdev_mcs = [
        (sem/3)*(r_mc/v_mc) for sem, r_mc, v_mc in zip(sems, r_mcs, v_mcs)
    ]
    fig, ax = plot_radius_accuracy(r, r_mcs, r_meshs, nsampless, t_mcs, t_meshs, rdev_mcs)
    fig.suptitle('Single Sphere of r={:.2f} Volume Accuracy'.format(r))
    plt.show()


def test_double_sphere_effective_radius():
    rs = [np.random.random() + 0.5, np.random.random() + 0.5]
    centers = [np.zeros(3), np.ones(3)*0.25+np.random.random(3)*0.1]
    v_anl = anl_2sphere_union_volume(rs[0], rs[1], np.linalg.norm(centers[0]-centers[1]))
    r_anl = (3*v_anl/(4*np.pi))**(1/3)
    # nsampless = [1e3, 1e4, 1e5, 2e5, 3e5]
    nsampless = [1e4, 1e5, 2e5]
    v_meshs, t_meshs = get_mesh_volume_samples(rs, centers, np.array(nsampless)*150)
    r_meshs = [((3*v_mesh/(4*np.pi))**(1/3)) for v_mesh in v_meshs]
    v_mcs, t_mcs, dev_mcs = get_mc_volume_samples(rs, centers, nsampless)
    sems = np.array([dev_mc/v_mc for dev_mc, v_mc in zip(dev_mcs, v_mcs)])
    r_mcs = [((3*v_mc/(4*np.pi))**(1/3)) for v_mc in v_mcs]
    ...
    fig, ax = plot_radius_accuracy(r_anl, r_mcs, r_meshs, nsampless, t_mcs, t_meshs, rdev_mcs)
...
# test_single_sphere_effective_radius()
test_double_sphere_effective_radius()
