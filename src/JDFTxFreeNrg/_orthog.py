...
def remove_parallel_vectors(vectors: list[NDArray[np.float64]], cutoff: float = 1e-4) -> list[NDArray[np.float64]]:
    try:
...
def _remove_parallel_vectors(vectors: list[NDArray[np.float64]], cutoff: float = 1e-4) -> list[NDArray[np.float64]]:
    parallel_pairs = []
...
def orthogonalize_vector_set_step(vectors: list[NDArray[np.float64]], idx: int, cutoff: float = 1e-4) -> list[NDArray[np.float64]]:
    try:
...
def _orthogonalize_vector_set_step(vectors: list[NDArray[np.float64]], idx: int, cutoff: float = 1e-4) -> list[NDArray[np.float64]]:
...
def remove_parallel_vectors_loop(vectors: list[NDArray[np.float64]], cutoff: float = 1e-4, maxstep: int = 10) -> list[NDArray[np.float64]]:
    try:
...
def _remove_parallel_vectors_loop(vectors: list[NDArray[np.float64]], cutoff: float = 1e-4, maxstep: int = 10) -> list[NDArray[np.float64]]:
    step = 0
    _vectors = vectors
    while step < maxstep:
        #print("step ", step)
...
def orthogonalize_projector(projector1: NDArray[np.float64]) -> NDArray[np.float64]:
    try:
...
def _orthogonalize_projector(projector1: NDArray[np.float64]) -> NDArray[np.float64]:
...
def safe_orthogonalize_projector(projector1: NDArray[np.float64]) -> NDArray[np.float64]:
    try:
...
def progressively_orthogonalize_vectors(vectors: list[NDArray[np.float64]]) -> list[NDArray[np.float64]]:
    try:
...
def _progressively_orthogonalize_vectors(vectors: list[NDArray[np.float64]]) -> list[NDArray[np.float64]]:
    ...
    for j in range(len(vectors)):
        ...
        for k in range(j+1, len(vectors)):
            ...
            overlap: np.float64 = np.dot(v1, v2)
            orth_v2 = v2 - overlap * v1
